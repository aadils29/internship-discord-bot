...
4. Commit and push this project. The workflow in `.github/workflows/check_jobs.yml` runs every two hours and can also be started with **Actions** → **Check internship feed** → **Run workflow**. It commits `data/posted_jobs.json` back to the repository so IDs persist between runs.
